...
public class GamePanel extends JPanel implements Runnable, KeyListener{
	...
	public void tick( ) {
		...
		ticks++;
		//tick represents the speed of the snake  
		...
		for(int i = 0 ; i < apples.size(); i++) {
			if(xCoor == apples.get(i).getxCoor() && yCoor == apples.get(i).getyCoor()) {
				score++;
				if(score<5)         // size before score 5
					size=size+3;
				else               // size after score >=5
					size=size+5;
		...
		for(int i = 0; i < snake.size() ; i++) {
			...
			if(xCoor == snake.get(i).getxCoor() && yCoor == snake.get(i).getyCoor()) {
				if(i != snake.size()-1) {
					System.out.println("GAME OVER (u cut urself)");
					printscore(score);
					stop();
				}
			}
		...
		if(xCoor < 0 || xCoor >49 || yCoor <0 || yCoor >49 ) {
			System.out.println("GAME OVER (u touched the border)");
			printscore(score);
			stop();
		}
